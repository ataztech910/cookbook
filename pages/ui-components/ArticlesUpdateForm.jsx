...
export default function ArticlesUpdateForm(props) {
  ...
  const resetStateValues = () => {
    const cleanValues = { ...initialValues, ...articlesRecord };
    setTitle(cleanValues.title);
    setDescription(cleanValues.description);
    setText(cleanValues.text);
    setUploaded_file_url(cleanValues.uploaded_file_url);
    setPublished(cleanValues.published);
    setScheduled_pubilsh_date(cleanValues.scheduled_pubilsh_date);
    setErrors({});
  };
  const [articlesRecord, setArticlesRecord] = React.useState(articles);
  React.useEffect(() => {
    const queryData = async () => {
      const record = id ? await DataStore.query(Articles, id) : articles;
  ...
  return (
      ...
      onSubmit={async (event) => {
        ...
        try {
          await DataStore.save(
            Articles.copyOf(articlesRecord, (updated) => {
    ...
    >
      ...
      <TextField
        ...
        isRequired={false}
        isReadOnly={false}
        defaultValue={uploaded_file_url}
        onChange={(e) => {
          let { value } = e.target;
          if (onChange) {
            const modelFields = {
      ...
      <TextField
        ...
        isReadOnly={false}
        type="date"
        defaultValue={scheduled_pubilsh_date}
        onChange={(e) => {
          let { value } = e.target;
          if (onChange) {
            const modelFields = {
